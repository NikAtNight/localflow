        ...
        Task {
            ...
            do {
                ...
                if cleanupEnabled, Settings.cleanupEnabled {
                    stageStart = Date()
                    do {
                        text = try await OllamaCleaner.clean(text, model: Settings.cleanupModel)
                        stderr.write(Data("S1-mini cleanup: \(elapsedMs(since: stageStart))ms (\(Settings.cleanupModel))\n".utf8))
                    } catch {
